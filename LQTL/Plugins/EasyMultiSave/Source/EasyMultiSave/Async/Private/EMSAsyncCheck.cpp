...
void UEMSAsyncCheck::StartCheck()
{
	...
	default:
		CheckResult = EMS->CheckSaveGameIntegrity(EDataLoadType::DATA_Object, EMS->SlotInfoSaveFile(), bCheckGameVersion);
...
{
	CheckResult = EMS->CheckSaveGameIntegrity(EDataLoadType::DATA_Player, EMS->PlayerSaveFile(), bCheckGameVersion);
	EMS->GetTimerManager().SetTimerForNextTick(this, &UEMSAsyncCheck::CheckLevel);
}

void UEMSAsyncCheck::CheckLevel()
{
	CheckResult = EMS->CheckSaveGameIntegrity(EDataLoadType::DATA_Level, EMS->ActorSaveFile(), bCheckGameVersion);
	EMS->GetTimerManager().SetTimerForNextTick(this, &UEMSAsyncCheck::CompleteCheck);
}

void UEMSAsyncCheck::CheckCustom()
{
	CheckResult = EMS->CheckSaveGameIntegrity(EDataLoadType::DATA_Object, EMS->CustomSaveFile(SaveFileName, FString()), bCheckGameVersion);
	EMS->GetTimerManager().SetTimerForNextTick(this, &UEMSAsyncCheck::CompleteCheck);
}

void UEMSAsyncCheck::CheckCustomSlot()
{
	CheckResult = EMS->CheckSaveGameIntegrity(EDataLoadType::DATA_Object, EMS->CustomSaveFile(SaveFileName, EMS->GetCurrentSaveGameName()), bCheckGameVersion);
	EMS->GetTimerManager().SetTimerForNextTick(this, &UEMSAsyncCheck::CompleteCheck);
}
