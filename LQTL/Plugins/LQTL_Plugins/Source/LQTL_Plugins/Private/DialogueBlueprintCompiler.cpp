...
{
	UDialogueBlueprint* DialogueBP = GetDialogueBlueprint(); // Lấy Blueprint hiện tại dưới dạng UDialogueBlueprint

	const bool bRecompilingOnLoad = Blueprint->bIsRegeneratingOnLoad; // Kiểm tra xem có đang tái biên dịch khi tải không (Super Imple dòng này)
	...
	if (!bIsSkeletonOnly)
	{
		...
		if (BPGClass && DialogueBP->DialogueTemplate)
		{
			...
			if (const UTWDialogueEditorSettings* DialogueSettings = GetDefault<UTWDialogueEditorSettings>())
			{
				...
				TMap<FName, UTWDialogueNode*> NodeMap;
				...
				for (auto& Node : DialogueBP->DialogueTemplate->GetNodes())
				{
					if (Node)
					{
						...
						if (/*DialogueSettings->bEnableWarnings*/ true)
						{
							if (/*DialogueSettings->bWarnMissingSoundCues*/ true )
							{
								if (!Node->Line.DisplayText.IsEmpty())
								{
