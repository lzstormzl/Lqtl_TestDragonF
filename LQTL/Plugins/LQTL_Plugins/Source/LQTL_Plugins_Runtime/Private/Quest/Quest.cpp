...
class USocialComponent* UQuest::GetOwningSocialComponent() const
{
	return OwningComp;
}
...
UWorld* UQuest::GetWorld() const
{
	if (HasAnyFlags(RF_ClassDefaultObject))
	{
 	...
 	if (GetOwningSocialComponent())
 	{
 		return GetOwningSocialComponent()->GetWorld();
 	}
...
bool UQuest::Initialize(class USocialComponent* InitializingComp, const FName& QuestStartID /*= NAME_None*/)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
...
void UQuest::OnQuestTaskProgressChanged(const UQuestTask* Task, const class UQuestBranch* Step, int32 CurrentProgress, int32 RequiredProgress)
{
...
void UQuest::OnQuestTaskCompleted(const UQuestTask* Task, const class UQuestBranch* Branch)
{
