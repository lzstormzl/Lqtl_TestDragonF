...
struct LQTL_PLUGINS_RUNTIME_API FQuestTaskInfo
{
GENERATED_BODY()

public:
	FQuestTaskInfo(UQuestDataTask* InEvent, const FString& InArgument, const int32 InQuantity, const bool bInHidden, const bool bInOptional) : Task(InEvent), Argument(InArgument), Quantity(InQuantity), bHidden(bInHidden), bOptional(bInOptional)
	{
		CurrentProgress = 0;
	};
...
/**Base class for states and branches in the quests state machine*/
 UCLASS()
 class LQTL_PLUGINS_RUNTIME_API UQuestNode : public UTaleWeaversNodeBase
 {
...
class LQTL_PLUGINS_RUNTIME_API UQuestState : public UQuestNode
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Quest State", meta = (AdvancedDisplay))
...
class LQTL_PLUGINS_RUNTIME_API UQuestBranch : public UQuestNode
{
...
public:
	...
	void OnQuestTaskComplete(class UQuestTask* Task);
	...
	*/
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Instanced, Category = "Task", meta = (AdvancedDisplay))
	TArray<class UQuestTask*> QuestTasks;
