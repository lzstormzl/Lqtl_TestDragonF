...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FiveParams(FOnQuestTaskProgressChanged, const UQuest*, Quest, const UQuestTask*, ProgressedTask, const class UQuestBranch*, Branch, int32, OldProgress, int32, NewProgress);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnQuestTaskCompleted, const UQuest*, Quest, const UQuestTask*, CompletedTask, const class UQuestBranch*, Branch);
...
class LQTL_PLUGINS_RUNTIME_API UQuest : public UObject
{
	GENERATED_BODY()
	
// protected:

	friend class UQuestTask;
 	friend class UQuestState;
 	friend class UQuestBranch;
 	friend class USocialComponent;
public:
	UQuest();
...
// 	//The current state the player is at in this quest
 ...
 public:
	...
	class USocialComponent* GetOwningSocialComponent() const;
	...
	virtual bool Initialize(class USocialComponent* InitializingComp, const FName& QuestStartID = NAME_None);
...
// 	/**Manually set the quest as succeeded. You'll need to provide some text for the UI as theres no node  the quest, you're manually succeeding it.*/
	...
	UFUNCTION()
		void OnQuestTaskProgressChanged(const UQuestTask* Task, const class UQuestBranch* Step, int32 CurrentProgress, int32 RequiredProgress);

	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "On Quest Objective Progress Made"))
	void BPOnQuestTaskProgressChanged(const UQuest* Quest, const UQuestTask* Task, const class UQuestBranch* Step, int32 CurrentProgress, int32 RequiredProgress);

	UFUNCTION()
	void OnQuestTaskCompleted(const UQuestTask* Task, const class UQuestBranch* Branch);

	UFUNCTION(BlueprintImplementableEvent, meta = (DisplayName = "On Quest Task Completed"))
		void BPOnQuestTaskCompleted(const UQuest* Quest, const UQuestTask* Task, const class UQuestBranch* Step);
