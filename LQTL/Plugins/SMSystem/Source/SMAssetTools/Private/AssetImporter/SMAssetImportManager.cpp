...
USMAssetImporter::EImportStatus FSMAssetImportManager::ImportAsset(const USMAssetImporter::FImportArgs& InImportArgs, USMAssetImporter* InImporter)
{
	...
	check(InImportArgs.ImportToBlueprint->GeneratedClass->GetDefaultObject(false));
	...
	{
		const USMAssetImporter::EImportStatus Status = InImporter->ImportCDO(InImportArgs.ImportToBlueprint->GeneratedClass->GetDefaultObject(false));
