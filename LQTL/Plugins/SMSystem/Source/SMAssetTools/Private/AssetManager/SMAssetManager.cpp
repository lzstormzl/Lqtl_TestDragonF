...
{
	check(InBlueprint && InBlueprint->GeneratedClass && InBlueprint->GeneratedClass->GetDefaultObject(false));
	check(InNewClassDefaults);

	ensureMsgf(InBlueprint->GeneratedClass->GetDefaultObject(false)->GetClass()->IsChildOf(InNewClassDefaults->GetClass()),
		TEXT("The CDO class is not equal to or is not a child of the new defaults."));

	UEngine::CopyPropertiesForUnrelatedObjects(InNewClassDefaults, InBlueprint->GeneratedClass->GetDefaultObject(false));
