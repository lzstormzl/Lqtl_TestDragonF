...
void FSMEditableTextLayout::InsertRunAtCursor(TSharedRef<IRun> InRun)
{
...
#if 0
	// For inserting the run(rich text) directly into text rather than relying on parsing.
	...
	FString RunText;
	InRun->AppendTextTo(RunText);
#else
	// For inserting the plain format text argument into text instead.

	FString RunText;
	ensure(FRunTypeUtils::TryGetRunNameAsFormatArgument(InRun, RunText));

	TextLayout->InsertAt(CursorInteractionPosition, RunText);
#endif
	const TArray<FTextLayout::FLineModel>& Lines = TextLayout->GetLineModels();
	const FTextLayout::FLineModel& Line = Lines[CursorInteractionPosition.GetLineIndex()];
	const FTextLocation FinalCursorLocation = FTextLocation(CursorInteractionPosition.GetLineIndex(), FMath::Min(CursorInteractionPosition.GetOffset() + RunText.Len(), Line.Text->Len()));
