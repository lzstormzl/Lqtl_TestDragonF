...
void USMTextPropertyGraph::NotifyGraphChanged(const FEdGraphEditAction& Action)
{
	...
	else if (Action.Action & EEdGraphActionType::GRAPHACTION_EditNode)
	{
		SetTextFromFormatTextNode();
	}
}
...
void USMTextPropertyGraph::RefreshTextBody(bool bModify, bool bResetGraph, bool bOnlyIfChanged)
{
	...
	if (bOnlyIfChanged)
	{
		...
		{
			if (ResultNode && ISMEditorConstructionManager::Get().IsRunningConstructionScripts(Blueprint))
			{
...
void USMTextPropertyGraph::SetTextBody(const FText& PlainText, bool bModify, bool bReformatGraph)
{
	...
	if (FormatTextNode != nullptr)
	{
		...
		if (bReformatGraph)
		{
			...
			for (const auto& Variable : ParserResult.Variables)
			{
				...
				FProperty* Property = FSMBlueprintEditorUtils::GetPropertyByName(Blueprint, Variable.Key);
				if (Property == nullptr)
				{
					FSMGraphNodeLog NodeLog(EMessageSeverity::Error);
	...
	if (!ISMEditorConstructionManager::Get().IsRunningConstructionScripts() && ResultNode && ResultNode->OwningGraphNode)
	{
		...
		for (int32 TextIndex = 0; TextIndex < NumTexts; ++TextIndex)
		{
			...
			if (!bIsStringTable)
			{
#if USE_STABLE_LOCALIZATION_KEYS
				{
					...
					if (NewKey != ProposedKey && NewNamespace == ProposedNamespace)
					{
						...
						const bool bIsLegacy = ConstructionProjectSetting == ESMEditorConstructionScriptProjectSetting::SM_Legacy;
						if (ResultNode->HasDefaultValueExplicitlyBeenChanged() || bIsLegacy)
						{
							...
							if (bIsLegacy)
							{
								LDEDITOR_TEXTGRAPH_LOG_WARNING(TEXT("Stable localization keys may not work properly with EditorNodeConstructionScriptSetting set to Legacy. Desired key: %s. Actual key: %s."), *ProposedKey, *NewKey);
							}
						}
					}
				}
		...
		if (const UEdGraphPin* FormatTextPin = GetFormatTextNodePinChecked())
		{
			...
			if (bForce || bStringTableOrGraphEdit || bLocalizationChange)
			{
					...
					FTextInspector::GetSourceString(FormatPinText) == &FStringTableEntry::GetPlaceholderSourceString())
				{
					...
					{
						const TWeakObjectPtr<USMTextPropertyGraph> WeakPtrThis(this);
						auto OnTableLoaded = [WeakPtrThis, this](FName TableRequested, FName TableLoaded)
						{
							if (GEditor && GEditor->IsTimerManagerValid() && WeakPtrThis.IsValid())
							{
								// Even though the string table is loaded, the FText properties won't have updated
								// display text until the next tick.
								GEditor->GetTimerManager()->SetTimerForNextTick(
									FTimerDelegate::CreateLambda([WeakPtrThis, this]()
								{
										if (!WeakPtrThis.IsValid() || !IsValid(FormatTextNode) ||
