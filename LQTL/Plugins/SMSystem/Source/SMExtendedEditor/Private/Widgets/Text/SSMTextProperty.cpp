...
SSMTextProperty::SSMTextProperty()
{
}
...
{
	if (FSMStateMachineBlueprintEditor* Editor = FSMBlueprintEditorUtils::GetStateMachineEditor(GraphNode.Get()))
	{
...
{
	if (const FSMStateMachineBlueprintEditor* Editor = FSMBlueprintEditorUtils::GetStateMachineEditor(GraphNode.Get()))
	{
		// Check that we're not in debug mode.
		if (!Editor->InEditingMode())
		{
