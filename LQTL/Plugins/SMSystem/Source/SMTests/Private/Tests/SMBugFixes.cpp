 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDuplicateRuntimeNodeTest, "LogicDriver.BugFixes.CheckDuplicateRuntimeNode",
...
bool FDuplicateRuntimeNodeTest::RunTest(const FString& Parameters)
{
	...
	// Set duplicate state nodes.
	{
		...
		FSMNode_Base* OriginalRuntimeNode = FSMBlueprintEditorUtils::GetRuntimeNodeFromGraph(StateNodes[1]->GetBoundGraph());

		FSMNode_Base* DupeRuntimeNode = FSMBlueprintEditorUtils::GetRuntimeNodeFromGraph(StateNodes[2]->GetBoundGraph());
		DupeRuntimeNode->SetNodeGuid(OriginalRuntimeNode->GetNodeGuid());
		FSMBlueprintEditorUtils::UpdateRuntimeNodeForGraph(DupeRuntimeNode, StateNodes[2]->GetBoundGraph());
	}
	...
	// Set duplicate state nodes.
	{
		...
		for (int32 i = 2; i < TotalStates + 1 /* Entry state counts so add 1 */; ++i)
		{
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FInvalidNodeNameTest, "LogicDriver.BugFixes.CheckInvalidNodeName",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
/**
 * Validate the localization fields haven't changed for looking up the gameplay tag editor for BP pins.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(GameplayTagPinEditorNameTest, "LogicDriver.BugFixes.GameplayTagPinEditorName",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool GameplayTagPinEditorNameTest::RunTest(const FString& Parameters)
{
	FText TagEditorName;
	const bool bFoundLocalizedName = FText::FindTextInLiveTable_Advanced(TEXT("GameplayTagContainerCombo"),
		TEXT("GameplayTagQueryEntryBox_WidgetTitle"), TagEditorName);

	TestTrue(TEXT("Found the localization for the gameplay tag editor title."), bFoundLocalizedName);
	TestTrue(TEXT("Name is not empty."), !TagEditorName.IsEmpty());
	
	return bFoundLocalizedName && !TagEditorName.IsEmpty();
}
