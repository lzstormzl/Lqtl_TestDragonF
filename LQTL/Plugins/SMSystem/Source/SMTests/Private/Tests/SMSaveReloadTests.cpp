...
bool TestSaveStateMachineState(FAutomationTestBase* Test, bool bCreateReferences, bool bReuseStates, bool bCreateIntermediateReferenceGraphs = false)
{
	...
	if (!TestHelpers::TryCreateNewStateMachineAsset(Test, NewAsset, false, USMTestNotifyInstance::StaticClass()))
	{
		return false;
	}
	...
	// Now increment its states testing that active/current state retrieval works properly.
	{
		...
		else
		{
			...
			USMBlueprintGeneratedClass* BPGC = NewBP->GetGeneratedClass();
			...
			Test->TestEqual("Reference template in BPGC", BPGC->GetSubObjectTemplates().Num(), TotalTemplates);
			int32 TemplatesVerified = 0;
			for (const TTuple<FName, TObjectPtr<UObject>>& TemplateObject : BPGC->GetSubObjectTemplates())
			{
				if (USMInstance* Template = Cast<USMInstance>(TemplateObject.Value))
				{
					Test->TestEqual("Template outered to BPGC", Template->GetOuter(), Cast<UObject>(BPGC));
			...
			Test->TestEqual("Templates verified in BPGC", TemplatesVerified, TotalTemplates);
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSaveStateMachineStateTest, "LogicDriver.SaveRestore.StateMachineState",
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSaveStateMachineStateWithReferencesTest, "LogicDriver.SaveRestore.StateMachineStateWithReferences",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FSaveStateMachineStateWithIntermediateReferencesTest, "LogicDriver.SaveRestore.StateMachineStateWithIntermediateReferences",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
