                                               ...
                                               const FString& InDesiredNamespace, const FString& InDesiredKey, const FString& InTemporaryKey)
{
	...
	// UE 5.5 - Look for a BPGC that contains a reference. This can add to the reference count when it should be treated as the
	// same object. This wasn't required when the outer used to be the CDO.
	if (RefCount > 1)
	{
		for (const TTuple<UObject*, TArray<FText*>>& Reference : ObjectsReferencing)
		{
			// This check might be able to be more specific, but there's no reason an outer should be the BPGC otherwise.
			if (Reference.Key
				&& Reference.Key->HasAnyFlags(RF_ArchetypeObject)
				&& Reference.Key->GetOuter()
				&& Reference.Key->GetOuter()->IsA<USMBlueprintGeneratedClass>())
			{
				RefCount--;
				break;
			}
