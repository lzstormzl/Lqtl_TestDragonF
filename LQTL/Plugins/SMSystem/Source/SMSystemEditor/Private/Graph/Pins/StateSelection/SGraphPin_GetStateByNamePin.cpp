...
{
	FEdGraphUtilities::RegisterVisualPinFactory(MakeShared<FSMGetStateByNamePinFactory>());
}
...
{
	return SAssignNew(StatePickerAnchor, SComboButton)
	.ContentPadding(3.f)
...
{
	StatePickerAnchor->SetIsOpen(false);
}
