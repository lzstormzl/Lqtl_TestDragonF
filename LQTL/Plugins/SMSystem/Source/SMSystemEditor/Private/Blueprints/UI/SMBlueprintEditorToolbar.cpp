...
{
	TSharedPtr<FSMStateMachineBlueprintEditor> EditorPtr = Editor.Pin();
	...
	TAttribute<FName> GetActiveMode(EditorPtr.ToSharedRef(), &FSMStateMachineBlueprintEditor::GetCurrentMode);
	FOnModeChangeRequested SetActiveMode = FOnModeChangeRequested::CreateSP(
		EditorPtr.ToSharedRef(), &FSMStateMachineBlueprintEditor::SetCurrentMode);
...
{
	const TSharedPtr<FSMStateMachineBlueprintEditor> EditorPtr = Editor.Pin();
	check(EditorPtr.IsValid());
