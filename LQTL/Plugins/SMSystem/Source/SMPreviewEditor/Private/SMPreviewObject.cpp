...
void USMPreviewObject::OnWorldDestroyed(UWorld* World)
{
	...
	if (SimulatedStateMachineActor && SimulatedStateMachineActor->GetWorld() == World)
	{
		// Null out the actor indicating that it should be respawned.
		SimulatedStateMachineActor = nullptr;
	}
}
...
USMInstance* USMPreviewObject::InitializeStateMachine(UObject* InContext)
{
	...
	SimulatedStateMachineInstance = USMBlueprintUtils::CreateStateMachineInstanceFromTemplate(StateMachineTemplate->GetClass(), InContext, StateMachineTemplate);
	SimulatedStateMachineInstance->SetTickBeforeBeginPlay(true);

	if (World && !SimulatedStateMachineActor)
	{
		SimulatedStateMachineActor = CastChecked<ASMPreviewStateMachineActor>(World->SpawnActor(ASMPreviewStateMachineActor::StaticClass()));
		SimulatedStateMachineActor->ClearFlags(RF_Public);
	}

	if (World && SimulatedStateMachineActor)
	{
		SimulatedStateMachineActor->StateMachineInstance = SimulatedStateMachineInstance;
	}
	
	return SimulatedStateMachineInstance;
}
...
void USMPreviewObject::ShutdownStateMachine()
{
	...
	if (SimulatedStateMachineActor)
	{
		SimulatedStateMachineActor->MarkAsGarbage();
		SimulatedStateMachineActor = nullptr;
	}
