	...
	*PathFilter, *PinTypeString, *FSMSearchTags::FiB_NodeData.ToString(), *FFindInBlueprintSearchTags::FiB_DefaultValue.ToString(), *FinalSearchString);
	...
	while (!InActiveSearch->StreamSearch->IsComplete())
	{
		...
		if (IsInGameThread())
		{
			...
			// open. Manually tick to ensure the process completes. Note we can only safely do this on the game thread in UE 5.6.
			// However, there have been underlying improvements to FiB since UE 5.5 which hopefully prevents the need of this on our end.
